...
├── app.js        # 전체 비즈니스 로직 (IIFE, ~320줄)
├── style.css     # CSS Custom Properties 기반 다크 테마 스타일링 (~420줄)
...
- **순서 변경**: ⇅ 버튼 토글 → 드래그 핸들(☰) 표시 → 마우스 이벤트 기반 드래그앤드롭
- **진행률**: 카테고리별 완료/전체 카운트 + 그라디언트 프로그레스 바 (카테고리별 색상)
